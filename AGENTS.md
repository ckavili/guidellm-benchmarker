...
This is `guidellm-benchmarker`, a community plugin for the **Red Hat OpenShift AI (RHOAI) Dashboard**. It uses Webpack 5 Module Federation to expose remote modules that the RHOAI dashboard host application loads at runtime.
...
- **`./extensions`** (`src/rhoai/extensions.ts`) — Defines seven extension points:
  - `app.area` — registers the `guidellm-benchmarker` feature area
  - `app.navigation/section` (x2) — `community-plugins` shared parent section (with `CommunityNavIcon`) and `guidellm-benchmarker` plugin subsection (with `GuidellmBenchmarkerNavIcon`)
  - `app.navigation/href` (x3) — "User Info", "Cluster Resources", and "Namespace Summary" nav items under the `guidellm-benchmarker` section
  - `app.route` — mounts the App component with wildcard routing at `/guidellm-benchmarker/*`
- **`./Icon`** (`src/app/components/GuidellmBenchmarkerNavIcon.tsx`) — SVG icon for the plugin's nav subsection. A separate `CommunityNavIcon.tsx` provides the icon for the shared `community-plugins` parent section.
...
The plugin has three pages, routed under `/guidellm-benchmarker/*`:
...
The `bff/` directory contains a standalone Express.js + TypeScript backend service that implements the BFF pattern. The dashboard proxies requests from `/guidellm-benchmarker/api/*` to this service, forwarding the user's Bearer token. See `docs/architecture/BFF_PATTERN.md` for details.
...
- `config/webpack.dev.js` — Dev server on port 9500, proxies `/guidellm-benchmarker/api` to BFF at `localhost:3000` and `/guidellm-benchmarker` to dashboard at `localhost:8443`
...
- **Helm chart**: `chart/` deploys to Kubernetes with Deployment + Service for both frontend and BFF. Frontend defaults to `quay.io/OWNER/guidellm-benchmarker:latest`, BFF to `quay.io/OWNER/guidellm-benchmarker-bff:latest`.
