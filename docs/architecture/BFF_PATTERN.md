...
Browser                    Dashboard Backend              Plugin BFF              K8s API
  |                              |                            |                     |
  |-- fetch('/guidellm-benchmarker/api/namespace-summary') ----------->|                     |
  |                              |                            |                     |
...
1. The frontend calls a path like `/guidellm-benchmarker/api/namespace-summary` at the same origin
...
```json
{
  ...
  "backend": {
    ...
    "service": { "name": "guidellm-benchmarker", "namespace": "guidellm-benchmarker", "port": 8080 }
  },
  "proxyService": [{
    ...
    "authorize": true,
    "tls": false,
    "service": { "name": "guidellm-benchmarker-bff", "namespace": "guidellm-benchmarker", "port": 3000 }
  }]
}
```
...
- **Deployment**: `guidellm-benchmarker-bff` -- Node.js container on port 3000
- **Service**: `guidellm-benchmarker-bff` -- ClusterIP service exposing port 3000
...
The dashboard must include a `proxyService` entry in `MODULE_FEDERATION_CONFIG` to route `/guidellm-benchmarker/api/*` requests to the BFF:
...
The webpack dev server (`config/webpack.dev.js`) also has a proxy entry for `/guidellm-benchmarker/api` that forwards to `localhost:3000`. This allows developing the frontend against the BFF without the full dashboard, but note that no user token will be forwarded in this mode.
