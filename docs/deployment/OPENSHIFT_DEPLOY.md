...
```bash
helm install guidellm-benchmarker oci://quay.io/OWNER/guidellm-benchmarker-chart \
  --version 0.1.0 \
  --namespace guidellm-benchmarker \
  --create-namespace
```
...
```bash
helm install guidellm-benchmarker chart/ \
  --namespace guidellm-benchmarker \
  --create-namespace
```
...
- A **Deployment** and **Service** (`guidellm-benchmarker`) serving the plugin's static assets (including `remoteEntry.js`) via Nginx on port 8080
- A **BFF Deployment** and **Service** (`guidellm-benchmarker-bff`) running the plugin's backend service on port 3000 (enabled by default)
...
The `proxyService` entry tells the dashboard to forward requests from `/guidellm-benchmarker/api/*` to the BFF service, rewriting the path to `/api/*` and forwarding the user's Bearer token (`authorize: true`).
...
```bash
oc get pods -n guidellm-benchmarker
```

You should see pods for `guidellm-benchmarker` (and `guidellm-benchmarker-bff` if BFF is enabled), all in `Running` status.
...
Retrieve the current config, remove the `guidellmBenchmarker` entry, and re-apply:
...
import json, sys
config = json.load(sys.stdin)
config = [e for e in config if e.get('name') != 'guidellmBenchmarker']
print(json.dumps(config))
...
```bash
helm uninstall guidellm-benchmarker -n guidellm-benchmarker
oc delete namespace guidellm-benchmarker   # optional: remove the namespace entirely
```
...
|---|---|---|
| `image.repository` | `quay.io/OWNER/guidellm-benchmarker` | Frontend container image |
...
| `bff.image.repository` | `quay.io/OWNER/guidellm-benchmarker-bff` | BFF container image |
