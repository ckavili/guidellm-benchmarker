...
|---|---|---|---|
| `package.json` | `name` | `guidellm-benchmarker` | `{your-plugin}` |
| `package.json` | `module-federation.name` | `guidellmBenchmarker` | `{yourPlugin}` (camelCase) |
| `package.json` | `module-federation.proxy[].path` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `package.json` | `module-federation.proxy[].pathRewrite` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `package.json` | `module-federation.local.port` | `9500` | Any unused port (see [Port allocation](#port-allocation)) |
| `plugin.yaml` | `name` | `guidellm-benchmarker` | `{your-plugin}` |
| `plugin.yaml` | `displayName` | `Guidellm Benchmarker` | Your plugin name |
| `plugin.yaml` | `image.repository` | `quay.io/.../guidellm-benchmarker` | Your image repository |
| `plugin.yaml` | `install.helm.registry` | `oci://quay.io/.../guidellm-benchmarker` | Your OCI chart registry |
| `plugin.yaml` | `remote.spec.name` | `guidellmBenchmarker` | `{yourPlugin}` (camelCase) |
| `plugin.yaml` | `remote.spec.scope` | `guidellmBenchmarker` | `{yourPlugin}` (must match `name`) |
| `plugin.yaml` | `remote.spec.remoteEntry` | `.../guidellm-benchmarker/...` | Your deployed image URL |
| `plugin.yaml` | `remote.spec.paths[0].path` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `plugin.yaml` | `remote.spec.paths[0].extensions` | `guidellmBenchmarker/extensions` | `{yourPlugin}/extensions` |
| `plugin.yaml` | `remote.spec.paths[1].path` | `guidellmBenchmarker/Icon` | `{yourPlugin}/Icon` |
| `src/rhoai/extensions.ts` | area `id` | `guidellm-benchmarker` | `{your-plugin}` |
| `src/rhoai/extensions.ts` | plugin section `id` | `guidellm-benchmarker` | `{your-plugin}` |
| `src/rhoai/extensions.ts` | plugin section `title` | `Guidellm Benchmarker` | Your plugin name |
| `src/rhoai/extensions.ts` | plugin section `group` | `1_guidellm_benchmarker` | `{N}_{your_plugin}` |
| `src/rhoai/extensions.ts` | nav item `id`s | `guidellm-benchmarker-*` | `{your-plugin}-{page}` |
| `src/rhoai/extensions.ts` | nav item `href`/`path` | `/guidellm-benchmarker/*` | `/{your-plugin}/*` |
| `src/rhoai/extensions.ts` | route `path` | `/guidellm-benchmarker/*` | `/{your-plugin}/*` |
| `src/bootstrap.tsx` | `Router basename` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `config/webpack.common.js` | MF plugin `name` | `guidellmBenchmarker` | `{yourPlugin}` (camelCase) |
| `config/moduleFederation.js` | `name` | `guidellmBenchmarker` | `{yourPlugin}` (camelCase) |
| `config/webpack.dev.js` | proxy `context` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `config/webpack.dev.js` | `port` | `9500` | Same as `package.json` port |
| `.env.development` | `URL_PREFIX` | `/guidellm-benchmarker` | `/{your-plugin}` |
| `chart/Chart.yaml` | `name` | `guidellm-benchmarker-chart` | `{your-plugin}-chart` |
| `chart/values.yaml` | `image.repository` | `quay.io/.../guidellm-benchmarker` | Your image repository |
| `chart/values.yaml` | `ingress.path` | `/guidellm-benchmarker` | `/{your-plugin}` |
