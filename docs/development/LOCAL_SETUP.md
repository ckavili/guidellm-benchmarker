...
> **Note:** The `proxyService` entry in the `MODULE_FEDERATION_CONFIG` (Step 2) is what tells the dashboard to forward `/guidellm-benchmarker/api/*` requests to the BFF at `localhost:3000`. If you omit the `proxyService` block, those requests will hit the dashboard's SPA fallback and return HTML instead of JSON.
...
Open the dashboard URL in your browser. You should see the RHOAI Dashboard with your plugin loaded in the sidebar, including the Namespace Summary page under the Guidellm Benchmarker section.

### How it works

- The backend reads `MODULE_FEDERATION_CONFIG`, registers a proxy `/_mf/guidellmBenchmarker/*` that routes to `http://localhost:9500/*`.
...
```bash
MODULE_FEDERATION_CONFIG=[{"name":"guidellmBenchmarker","backend":{"remoteEntry":"/remoteEntry.js","tls":false,"localService":{"host":"localhost","port":9500},"service":{"name":"placeholder","namespace":"opendatahub","port":8080}},"proxyService":[{"path":"/guidellm-benchmarker/api","pathRewrite":"/api","authorize":true,"tls":false,"localService":{"host":"localhost","port":3000},"service":{"name":"placeholder","namespace":"opendatahub","port":3000}}]}]
```
...
> **Note:** The `proxyService` entry in `env.local` (Step 6) tells the dashboard to forward `/guidellm-benchmarker/api/*` requests to the BFF. Without it, those requests return HTML instead of JSON.
...
| Field | Description |
|---|---|
| `proxyService[].path` | URL path prefix the dashboard intercepts (e.g. `/guidellm-benchmarker/api`). |
...
The frontend receives HTML instead of JSON. This means the request to `/guidellm-benchmarker/api/*` is not being proxied to the BFF and is hitting the SPA fallback instead.
