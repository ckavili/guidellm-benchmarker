# Guidellm Benchmarker

A community plugin for the **Red Hat OpenShift AI (RHOAI) Dashboard**. It uses Webpack 5 Module Federation to integrate with the dashboard at runtime.

## What's Inside

The plugin provides three pages, each using a different integration pattern:
...
This creates a Deployment and Service for both the frontend (`guidellm-benchmarker`, serving `remoteEntry.js` via Nginx) and the BFF (`guidellm-benchmarker-bff`, Node.js backend on port 3000). To deploy the frontend only, add `--set bff.enabled=false`.
...
You should see something like the following (note `guidellmBenchmarker` at the bottom of the list):
...
```bash
git clone https://github.com/rh-ai-community-plugins/guidellm-benchmarker.git my-plugin
cd my-plugin
rm -rf .git
git init
```
